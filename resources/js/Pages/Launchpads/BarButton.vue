...
<template>
	<Link class="px-2" v-if="launch?.contract" :href="route('launchpads.show', { launchpad: launch?.contract })">
	<div
		class="bg-black/20 border border-white/10 flex items-center gap-2 py-1.5 px-3 rounded-xl transition-colors duration-200">
		...
		<div class="flex items-center text-xs font-semibold text-[#f0b90b]">
			<svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 448 512" focusable="false"
		...
		<div class="text-[#68D391] uppercase flex items-center tracking-widest text-[10px]">
			<Clock class="w-3 h-3 mr-0.5" />
			<UseTimeAgo v-slot="time" :time="DateTime.fromFormat(
				launch.created_at,
				'yyyy-MM-dd HH:mm:ss',
			).toJSDate()
				">
